...
class SparseConv2DFunction(function.Function):
...
#            Wb_mat = _kern()(W_mat)
            ...
            W_sparse_mat = W_plus_mat + W_minus_mat

#            numpy.set_printoptions(threshold=numpy.inf)
#            print(dir(W_sparse_mat))
#            print(cuda.to_cpu(W_sparse_mat[0:16,0:9]).copy())
#            print(W_sparse_mat[0:16,0:9].shape)
#            exit()
...
#                y_mats[i] = Wb_mat.dot(col_mats[i])
                y_mats[i] = W_sparse_mat.dot(col_mats[i])
...
def func_convolution_2d(x, W, b=None, stride=1, pad=0, use_cudnn=True,
    ...
    """
    func = SparseConv2DFunction(stride, pad, use_cudnn, cover_all)
    if b is None:
        return func(x, W)
    else:
        return func(x, W, b)
