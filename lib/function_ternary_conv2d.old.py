...
def _kern_plus():
    return cuda.elementwise(
        'T x', 'T y',
#        'y = x > 0.1 ? x : 0', # yolov2_train
        'y = x > 0.001 ? 1 : 0', # darknet19_train
        'binarize')
...
class TernaryConv2DFunction(function.Function):
...
#        Wb = numpy.where(W>=0,1,-1).astype(W.dtype, copy=False)
        ...
        W_plus = numpy.where(W>0.001,W,0).astype(W.dtype, copy=False)
        W_minus = numpy.where(W<-0.001,W,0).astype(W.dtype, copy=False)
        Wb = W_plus + W_minus
        Xb = self.col #W # numpy.where(W>=0,1,-1).astype(W.dtype, copy=False)
...
#            Wb_mat = _kern()(W_mat)
            W_plus_mat = _kern_plus()(W_mat)
            W_minus_mat = _kern_minus()(W_mat)

            W_ternary_mat = W_plus_mat + W_minus_mat

#            numpy.set_printoptions(threshold=numpy.inf)
#            print(dir(W_ternary_mat))
#            print(cuda.to_cpu(W_ternary_mat[0:16,0:9]).copy())
#            print(W_ternary_mat[0:16,0:9].shape)
#            exit()
...
#                y_mats[i] = Wb_mat.dot(col_mats[i])
                y_mats[i] = W_ternary_mat.dot(col_mats[i])

#                print(col_mats[i])
#                print(col_mats[i].shape)
...
def func_convolution_2d(x, W, b=None, stride=1, pad=0, use_cudnn=True,
    ...
    """
    func = TernaryConv2DFunction(stride, pad, use_cudnn, cover_all)
    if b is None:
        return func(x, W)
    else:
        return func(x, W, b)
