...
def random_overlay_image(src_image, overlay_image, minimum_crop):
    ...
    sch = src_h-scale_item_h
    scw = src_w-scale_item_w
...
#    print("sch=%d scw=%d" % (sch,scw))
    rand_h = np.random.randint(sch)
    rand_x = np.random.randint(scw)
  
    y = int( rand_h - shift_item_h)
    x = int( rand_x - shift_item_w)
...
class ImageGenerator():
    ...
    def generate_random_animation(self, loop, bg_index, crop_width, crop_height, min_item_scale, max_item_scale):
        ...
        for i in range(loop):
            #class_id = np.random.randint(len(self.labels)) # comment out by a developer
...
#            print(self.bgs) # backgrounds
#            print("len(self.bgs)=%d" % (len(self.bgs)))
            val = np.random.randint(len(self.bgs))
#            print("val=%d" % val)
            bg = self.bgs[val]
